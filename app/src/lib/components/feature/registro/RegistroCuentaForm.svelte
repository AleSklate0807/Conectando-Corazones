<script lang="ts">
	...
	function leerFormularioPersistido(): InstantaneaFormulario | null {
		...
		try {
			const raw = window.sessionStorage.getItem(REGISTRO_STORAGE_KEY);
			if (!raw) {
				return null;
			}
	...
	function guardarFormularioPersistido(instantanea: InstantaneaFormulario) {
		...
		try {
			window.sessionStorage.setItem(REGISTRO_STORAGE_KEY, JSON.stringify(instantanea));
		} catch (error) {
						...
						<div class="flex items-start gap-3">
							{#if metodo.id === 'manual'}
								...
								>
									...
									>
										<KeyRound class="h-4 w-4 text-[#2563EB]" strokeWidth={1.7} />
									</span>
								</span>
							{:else}
								<span
								...
								>
									...
									>
										{#if metodoAcceso === 'federado' && metodo.disponible}
											<ShieldCheck class="h-4 w-4 text-emerald-500" strokeWidth={1.7} />
										{:else}
											<Clock class="h-4 w-4 text-amber-500" strokeWidth={1.7} />
										{/if}
									</span>
								</span>
							{/if}
				...
				<div class="space-y-6 rounded-2xl bg-white p-6">
					...
					{#if tipoInstitucionSeleccion === 'otro'}
						<CampoFormulario
							...
							label="Especifíca el tipo de institución"
							required
		...
		>
			<div class=" flex items-center gap-3">
				<span class="inline-flex items-center text-[rgb(var(--color-primary))]" aria-hidden="true">
					<LockKeyhole class="h-5 w-5" strokeWidth={1.6} />
				</span>
				<div class="flex-1">
