<script lang="ts">
	...
	import { X, School, MapPin, Star, Award, TrendingUp, UserPlus, Heart } from 'lucide-svelte';
		...
		>
			...
			>
				<div class="flex items-center gap-3">
					...
					<div>
						<h2 class="text-lg font-bold text-white sm:text-xl">Mi Impacto Social</h2>
						<p class="text-xs text-slate-400">Resumen de tu huella en la comunidad</p>
					</div>
				</div>
				<button
					onclick={onClose}
			...
			<div class="custom-scrollbar flex-1 overflow-y-auto p-5 sm:p-8">
				...
				<div class="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-3">
					...
					>
						...
						<div class="text-xs text-slate-400">Instituciones apoyadas</div>
						<div
							class="absolute -right-4 -bottom-4 h-24 w-24 rounded-full bg-blue-500/5 blur-xl"
						></div>
					</div>

					<!-- KPI 2: Nuevas este mes -->
					<div
						class="relative overflow-hidden rounded-2xl border border-white/5 bg-white/5 p-5 transition-transform hover:-translate-y-1"
					>
						<div class="mb-4 flex items-center justify-between">
					...
					>
						<div class="mb-4 flex items-center justify-between">
							...
							<span class="text-xs text-slate-500">Recurrencia</span>
						</div>
						<div class="mb-1 text-3xl font-bold text-white">{stats.retencion}%</div>
						<div class="text-xs text-slate-400">Nivel de compromiso</div>
						<div
				...
				<div class="mt-8 rounded-2xl border border-white/5 bg-white/5 p-6">
					...
					<div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
						{#each stats.topColaboradores as institucion}
							<div
							...
							>
								<div class="relative shrink-0">
									...
									>
										{#if institucion.avatarUrl}
											<img
												src={institucion.avatarUrl}
										...
										{:else}
											...
											>
												{institucion.nombre.substring(0, 2).toUpperCase()}
											</div>
										{/if}
									</div>
								</div>
								<div class="min-w-0 flex-1">
									...
									<div class="truncate text-xs text-slate-400">{institucion.rol}</div>
									<div
										class="mt-1 flex w-fit items-center gap-1 rounded-full bg-white/5 px-2 py-0.5 text-[10px] text-slate-300"
									>
										<span class="font-semibold text-blue-400">{institucion.aportes}</span> aportes
									</div>
								</div>
							</div>
						{/each}
